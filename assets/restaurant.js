(async function () {
  ...
  function statusBadge(status) {
    ...
    if (status === "paid" || status === "preparing" || status === "ready" || status === "completed") {
      return `<span class="pill badge-green">${status.toUpperCase()}</span>`;
    }
  ...
  function renderActive() {
    const orders = FC.ordersForRestaurant(restaurantId).filter(o =>
      ["approved", "awaiting_payment", "paid", "preparing", "ready"].includes(o.status)
    );
    ...
    for (const o of orders) {
      ...
      prep.onclick = () => {
        ...
        FC.log(`Order ${o.id} → preparing.`);
        renderAll();
      };

      ready.onclick = () => {
        FC.updateOrder(o.id, { status: "ready" });
        FC.log(`Order ${o.id} → ready.`);
        renderAll();
      };

      done.onclick = () => {
        FC.updateOrder(o.id, { status: "completed" });
        FC.log(`Order ${o.id} → completed.`);
        renderAll();
      };
